...
const EXERCISES=[
 ...
 {name:'Remo amb barra en banc · agafada prona',series:4,reps:8,sources:'Sessió 10',rest:120,note:'PES MAX (REGISTRAR)'},
 {name:'Hip thrust en banc',series:4,reps:8,sources:'Sessió 10',rest:120,note:'PES MAX (REGISTRAR)'},
 ...
 return <AppShell title="BASE · FORÇA">
  ...
  <div className="space-y-3">
   {grouped.map((ex,index)=>{const v=values[ex.name]||{};return <article key={ex.name} className="rounded-3xl border border-orange-100 bg-white p-4 shadow-sm">
    ...
    <div className="mt-2 rounded-xl bg-slate-50 px-3 py-2 text-xs font-bold text-slate-500">Sèries: {ex.series} · Descans entre sèries: <span className="text-orange-600">{ex.rest ? `${ex.rest} segons` : 'PENDENT DE CONFIRMAR'}</span>{ex.note && <span className="ml-2 font-black">· {ex.note}</span>}</div>
   </article>})}
  </div>
 </AppShell>
}