...
export default function AiPage() {
    ...
    const ask = (text) => {
        ...
        const adaptiveContext = [
            '[BOMBER COACH — PROGRESSIÓ ADAPTATIVA]',
            adaptive.principle,
            ...adaptive.recommendations.map((r) => `${r.title}: ${r.detail}`),
            '[FI PROGRESSIÓ ADAPTATIVA]',
        ].join('\n');
        const coachContext = [
            ...
            'Els barems indicats com inferits NO són oficials.',
            '[FI BOMBER COACH]',
        ].join('\n');
        sendMessage(`${text}\n\n${userContext}\n\n${bomberContext}\n\n${adaptiveContext}\n\n${coachContext}`);
        setInput('');
    ...
    return (
        <AppShell title="Assistent IA">
            <Helmet>
                ...
                <meta name="description" content="Assistent IA que analitza el teu historial real i adapta l'entrenament de Bombers." />
            </Helmet>
            ...
            <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
                ...
                <div className="mt-3 space-y-2">
                    {adaptive.recommendations.map((r, i) => (
                        <div key={`${r.type}-${i}`} className="rounded-2xl bg-slate-50 p-3">
            ...
            <div className="space-y-3 rounded-3xl bg-white border border-slate-200 p-4 shadow-sm min-h-[240px]">
                ...
                {messages.map((m, i) => (
                    <div key={i} className={m.role === 'user' ? 'text-right' : 'text-left'}><div className={`inline-block max-w-[90%] whitespace-pre-wrap rounded-2xl px-4 py-2 text-sm ${m.role === 'user' ? 'bg-slate-900 text-white' : 'bg-slate-100 text-slate-800'}`}>{m.role === 'user' ? m.content.split('\n\n[DADES')[0] : m.content}</div></div>
                ))}
                <div ref={endRef} />
            </div>
