...
function localCoachAnswer(question, data) {
  ...
  const has = (...terms) => terms.some(t => q.includes(normalizeQuestion(t)));
  if (has('hola', 'bon dia', 'bona tarda', 'bona nit', 'ei', 'hey')) return `Hola!\n\nSoc el Bomber Coach local. Tinc les teves dades disponibles i puc analitzar el teu entrenament.\n\nAra mateix la prioritat detectada és **${focus}**.`;
  ...
  return `Anàlisi local\n\nHe revisat les dades disponibles de Bomber Trainer. La prioritat detectada és **${focus}**.\n\nPuc analitzar el teu progrés, punts febles, evolució, entrenaments recents, distància, ritme, FC i temps des de l'últim treball.`;
}
...
function engineMeta(engine) { return engine === 'gemini' ? { label: 'GEMINI', icon: '🔵', className: 'bg-blue-50 text-blue-700 border-blue-200' } : { label: 'IA LOCAL · COACH LOCAL', icon: '🟢', className: 'bg-emerald-50 text-emerald-700 border-emerald-200' }; }

export default function AiPage() {
  const ownerId = pb.authStore.record?.id || 'guest';
  const [sessions, setSessions] = useState([]), [weights, setWeights] = useState([]), [goals, setGoals] = useState([]), [material, setMaterial] = useState([]), [minutes, setMinutes] = useState('');
  ...
  const [input, setInput] = useState(''), [loading, setLoading] = useState(false); const endRef = useRef(null);
  ...
  async function ask(question) {
    ...
    const route = routeAiQuestion(text, data); const engine = route.engine === 'gemini' ? 'gemini' : 'local';
    setMessages(prev => [...prev, { role: 'user', content: text }, { role: 'assistant', content: '', engine }]); setInput(''); setLoading(true);
    try {
      if (engine === 'local') {
        const answer = localCoachAnswer(text, data);
        setMessages(prev => { const copy = [...prev]; copy[copy.length - 1] = { role: 'assistant', content: answer, engine }; return copy; });
        return;
      }
      ...
      const answer = JSON.parse(raw)?.answer; if (!answer) throw new Error('Gemini ha retornat una resposta buida.');
      setMessages(prev => { const copy = [...prev]; copy[copy.length - 1] = { role: 'assistant', content: answer, engine }; return copy; });
    } catch (error) {
      setMessages(prev => { const copy = [...prev]; copy[copy.length - 1] = { role: 'assistant', content: `No he pogut completar la consulta.\n\n${error.message}`, engine }; return copy; });
    } finally { setLoading(false); }
  }
  function clearConversation() { setMessages([]); localStorage.removeItem(chatStorageKey(ownerId)); }

  return <AppShell title="Assistent IA"><Helmet><title>Bomber Coach — BOMBER TRAINER</title></Helmet>
    <div className="rounded-3xl p-5" style={{ backgroundColor: '#f3e8ff', borderLeft: '8px solid #7c3aed' }}><p className="text-xs font-bold tracking-widest text-purple-700">BOMBER COACH</p><p className="mt-2 text-sm text-slate-700">El router decideix automàticament quin motor respon cada pregunta.</p><div className="mt-2 flex items-center justify-between gap-2"><p className="text-xs font-semibold text-purple-700">{sessions.length} sessions</p>{messages.length > 0 && <button type="button" onClick={clearConversation} className="rounded-xl bg-white/80 px-3 py-2 text-xs font-bold text-slate-700 border border-purple-100">🗑️ Esborrar conversa</button>}</div></div>
    ...
    <div className="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-700"><b>Com funciona:</b> preguntes sobre les teves dades → <span className="font-semibold text-emerald-700">IA local</span>. Preguntes generals o de coneixement → <span className="font-semibold text-blue-700">Gemini</span>.</div>
    <div className="grid grid-cols-2 gap-2">{QUICK.map(q => <button key={q} onClick={() => ask(q)} disabled={loading} className="min-h-[52px] rounded-xl bg-white border border-slate-200 px-3 text-sm font-semibold shadow-sm">{q}</button>)}</div>
    <div className="space-y-3 rounded-3xl bg-white border border-slate-200 p-4 shadow-sm min-h-[300px]">
      {messages.length === 0 && <div className="rounded-2xl bg-purple-50 p-4 text-sm text-slate-700"><b>Hola!</b><br /><br />Fes una pregunta. El router decidirà si respon la <b>IA local</b> o <b>Gemini</b>, i t'ho indicaré en cada resposta.</div>}
      {messages.map((m, i) => { const meta = m.role === 'assistant' ? engineMeta(m.engine) : null; return <div key={i} className={m.role === 'user' ? 'text-right' : 'text-left'}>{m.role === 'assistant' && m.content && <div className={`mb-1 inline-flex items-center rounded-full border px-2.5 py-1 text-[11px] font-bold ${meta.className}`}>{meta.icon} {meta.label}</div>}<div className={`inline-block max-w-[92%] whitespace-pre-wrap rounded-2xl px-4 py-3 text-sm leading-6 ${m.role === 'user' ? 'bg-slate-900 text-white' : 'bg-slate-100 text-slate-800'}`}>{m.content || (loading && i === messages.length - 1 ? 'Pensant…' : '')}</div></div>; })}
      <div ref={endRef} />
    </div>
    <form onSubmit={e => { e.preventDefault(); ask(input); }} className="sticky bottom-24 flex gap-2 rounded-3xl bg-white border border-slate-200 p-3 shadow-sm"><input value={input} onChange={e => setInput(e.target.value)} placeholder="Pregunta al Bomber Coach…" className="min-h-[48px] flex-1 rounded-xl border border-slate-300 px-4" /><button type="submit" disabled={loading} className="min-h-[48px] rounded-xl bg-purple-700 px-5 font-bold text-white">{loading ? '…' : 'Envia'}</button></form>
  </AppShell>;
}
