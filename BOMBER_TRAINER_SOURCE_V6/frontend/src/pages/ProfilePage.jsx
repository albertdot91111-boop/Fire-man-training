...
function friendlyError(error) {
    ...
    if (status === 429) return 'El servidor està temporalment limitat. Les dades no s’han perdut; torna-ho a provar en uns segons.';
    if (status === 401 || status === 403) return 'Sessió no autoritzada. Torna a iniciar sessió.';
    if (status >= 500) return 'El servidor està temporalment ocupat. Torna-ho a provar en uns segons.';
    return error?.response?.message || error?.message || 'No s’han pogut carregar les dades.';
}
...
export default function ProfilePage() {
    ...
    return <AppShell title="El meu perfil">
        ...
        <section className="rounded-3xl bg-white border border-slate-200 shadow-sm p-5"><div className="flex items-center justify-between"><div><p className="text-xs font-bold tracking-widest text-slate-400">PROVES</p><h2 className="mt-1 text-lg font-black">Estat de preparació</h2></div><Target className="h-5 w-5 text-slate-300" /></div><div className="mt-4 space-y-2">{SPORT_META.map(([type, label]) => { const last = lastByType[type]; const age = daysSince(last?.date); const color = TYPES[type]?.color || '#0f172a'; return <div key={type} className="flex items-center gap-3 rounded-2xl bg-slate-50 p-3"><span className="h-9 w-9 rounded-xl flex items-center justify-center text-xs font-black text-white" style={{ background: color }}>{TYPES[type]?.short?.slice(0, 2)}</span><div className="min-w-0 flex-1"><p className="font-extrabold text-sm">{label}</p><p className="text-xs text-slate-500">{last ? `Últim entrenament: ${String(last.date).slice(0, 10)}` : 'Encara sense registres'}</p></div><span className={`text-xs font-bold ${age === null || age > 14 ? 'text-amber-600' : 'text-slate-500'}`}>{age === null ? 'Pendent' : age === 0 ? 'Avui' : `fa ${age} d`}</span></div>; })}</div></section>
