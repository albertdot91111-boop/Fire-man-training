...
export default function ProfilePage() {
    ...
    const totalSeconds = useMemo(() => sessions.reduce((sum, s) => sum + (Number(s.duration) || 0) * 60, 0), [sessions]);
    const totalKm = useMemo(() => sessions.reduce((sum, s) => sum + (Number(s.distance) || 0), 0), [sessions]);
    const activeDays = useMemo(() => new Set(sessions.filter((s) => s.type !== 'descans').map((s) => String(s.date || '').slice(0, 10))).size, [sessions]);
    ...
    return <AppShell title="El meu perfil">
        ...
        <section className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
                ...
                [Trophy, sessions.length, 'sessions'],
