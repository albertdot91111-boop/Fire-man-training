    ...
    return (
        <AppShell title="Configuració">
            <Helmet>
                ...
                <meta name="description" content="Material disponible, perfil i Suunto." />
            </Helmet>
            ...
            <section className="rounded-3xl bg-white border border-slate-200 p-5 shadow-sm">
                ...
                {suuntoMetrics && (
                    <div className="mt-4 grid grid-cols-2 gap-2 text-sm">
                        <div className="rounded-xl bg-slate-50 p-3"><span className="text-slate-500">FC mitjana</span><strong className="block text-lg">{suuntoMetrics.heartRate.average ?? '—'}{suuntoMetrics.heartRate.average ? ' bpm' : ''}</strong></div>
                        <div className="rounded-xl bg-slate-50 p-3"><span className="text-slate-500">FC màxima</span><strong className="block text-lg">{suuntoMetrics.heartRate.max ?? '—'}{suuntoMetrics.heartRate.max ? ' bpm' : ''}</strong></div>
                        ...
                        <div className="rounded-xl bg-slate-50 p-3"><span className="text-slate-500">Distància</span><strong className="block text-lg">{suuntoMetrics.distanceKm ?? '—'}{suuntoMetrics.distanceKm !== null ? ' km' : ''}</strong></div>
                    </div>
                )}
                <p className="mt-3 text-xs text-slate-400">La primera versió usa FIT del Suunto. La sincronització automàtica amb Suunto la farem en una segona fase.</p>
            </section>
