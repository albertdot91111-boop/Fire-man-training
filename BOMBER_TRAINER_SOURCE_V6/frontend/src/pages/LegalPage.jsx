...
function Privacy() {
  return <><Section title="Informació bàsica de protecció de dades">
    <p><strong>Responsable del tractament:</strong> el responsable del tractament identificat legalment per a l’activitat. <strong>Contacte:</strong> canal de contacte disponible a l’app.</p>
    ...
    <p><strong>Destinataris i proveïdors:</strong> les dades poden ser tractades pels proveïdors tecnològics necessaris per allotjar, autenticar, emmagatzemar i prestar el servei, amb les garanties contractuals i legals aplicables. L’app pot utilitzar PocketBase per a l’autenticació i emmagatzematge, Vercel per al desplegament/servei web i Google si l’usuari escull l’inici de sessió amb Google, i Google Gemini quan l’usuari utilitza les funcions d’IA que requereixen un servei extern. En aquestes funcions, el context necessari pot incloure preguntes, historial de conversa i dades d’entrenament, progressió, objectius, pes o activitats sincronitzades. Si l’usuari connecta voluntàriament serveis externs com Intervals.icu, les dades importades es tracten també segons la informació i configuració d’aquests serveis.</p><p><strong>Accés per part del responsable:</strong> la informació que introdueixis a BOMBER TRAINER pot ser accessible al responsable de l'aplicació quan sigui necessari per gestionar i mantenir el servei, donar suport, resoldre incidències, garantir la seguretat o complir obligacions legals. Aquest accés es limita a les finalitats legítimes del servei i no implica que les dades es facin públiques ni es comparteixin indiscriminadament amb tercers.</p>
    <p><strong>Conservació:</strong> durant el temps necessari per prestar el servei i, posteriorment, durant els terminis exigibles per obligacions legals o per determinar responsabilitats. Els registres tècnics d’accés i seguretat poden conservar-se durant el temps necessari per prevenir abusos, investigar incidències i acreditar la seguretat del servei.</p>
    <p><strong>Drets:</strong> pots sol·licitar accés, rectificació, supressió, limitació, oposició i, quan sigui aplicable, portabilitat. També pots retirar un consentiment en qualsevol moment. A més, des de <strong>Configuració → Privacitat i les meves dades</strong> pots descarregar les dades associades al teu compte o sol·licitar-ne l’eliminació mitjançant l’eliminació del compte, quan aquesta funcionalitat sigui aplicable. Per a qualsevol exercici de drets o incidència, caldrà utilitzar el canal de contacte que identifiqui el responsable a la versió publicada de la política.</p>
...
export default function LegalPage() {
  ...
  return <AppShell title={title}><div className="space-y-4">
    ...
    <p className="text-[11px] leading-5 text-slate-400">Nota: abans de publicar l’app, el titular ha de completar les dades identificatives i de contacte exigides per la normativa i revisar els proveïdors, transferències internacionals, terminis de conservació i configuració real de cookies/analítica. Així mateix, en el marc de les tasques d'administració, supervisió, millora i anàlisi del servei, el responsable podrà accedir a la informació d'activitat associada als comptes, incloent-hi, quan existeixi, registres d'entrenament, resultats, marques, puntuacions, progressió i altres indicadors de rendiment o nivell, amb la finalitat de comprendre l'ús de les funcionalitats i l'evolució global del servei. Aquest tractament es realitzarà d'acord amb les finalitats i garanties descrites en aquesta política.</p>
  </div></AppShell>;
}
