...
function timeSeries(sessions, type, totalFn) {
    return sessions
        ...
        .map((s) => ({ date: String(s.date || '').slice(5, 10), fullDate: String(s.date || '').slice(0, 10), seconds: totalFn(s) }))
...
export default function ProgressPage() {
    ...
    const load = async () => {
        ...
        try {
            const sessionRows = await pb.collection('bt_sessions').getFullList({ sort: '-date', filter: ownerFilter });
        ...
        try {
            const weightRows = await pb.collection('bt_weights').getFullList({ sort: '-date', filter: ownerFilter });
        ...
        try {
            const goalRows = await pb.collection('bt_goals').getFullList({ sort: '-created', filter: ownerFilter });
            setGoals(goalRows);
        } catch (err) {
            setGoals([]);
        }
