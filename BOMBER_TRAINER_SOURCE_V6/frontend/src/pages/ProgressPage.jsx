...
export default function ProgressPage() {
    ...
    const bestBenchResult = useMemo(() => sessions.flatMap((s) => Array.isArray(s.data) ? s.data : []).filter((e) => String(e.exercici || '').trim().toLowerCase() === 'press banca').map((e) => ({ weight: Number(e.pes) || 0, reps: Number(e.reps ?? e.repeticions) || 0, timeSeconds: Number(e.temps) || 0 })).filter((e) => e.weight > 0 && e.reps > 0).sort((a, b) => Math.min(b.weight / 65, b.reps / 12) - Math.min(a.weight / 65, a.reps / 12))[0] || null, [sessions]);
