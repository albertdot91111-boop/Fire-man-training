...
import WeeklyMaintenanceSummary from '@/components/WeeklyMaintenanceSummary';
import { TYPES, formatTime, gradeForBench, gradeForTime, levelFor, maintenanceEvolution, streak, today, totalPoints, officialPhysicalTime, PHYSICAL_PENALTY_SECONDS, physicalAverage } from '@/lib/btData';
...
function wearableMetrics(session) {
    const w = session?.wearable;
    if (!w || typeof w !== 'object') return null;
    const durationSeconds = metricNumber(w.durationSeconds);
    const distanceKm = metricNumber(w.distanceKm) || (metricNumber(w.distanceMeters) ? metricNumber(w.distanceMeters) / 1000 : null);
    const averageHeartRate = metricNumber(w.heartRate?.average);
    const maxHeartRate = metricNumber(w.heartRate?.max);
    const paceSeconds = durationSeconds && distanceKm ? durationSeconds / distanceKm : null;
    const pace = paceSeconds && paceSeconds > 0 && paceSeconds <= 3600
        ...
        : null;
    if (!durationSeconds && !distanceKm && !averageHeartRate && !maxHeartRate) return null;
    return { durationSeconds, distanceKm, pace, averageHeartRate, maxHeartRate };
}

function wearableActivitySeries(sessions) {
    return sessions.map((s) => {
        const metrics = wearableMetrics(s);
        if (!metrics) return null;
        return {
            date: String(s.date || '').slice(5, 10),
            fullDate: String(s.date || '').slice(0, 10),
            pointLabel: String(s.date || '').slice(5, 10),
            seconds: metrics.durationSeconds || 0,
            distanceKm: metrics.distanceKm,
            pace: metrics.pace,
            averageHeartRate: metrics.averageHeartRate,
            maxHeartRate: metrics.maxHeartRate,
            activityType: s.wearable?.activityType || s.wearable?.name || 'Activitat',
        };
    }).filter((item) => item && (item.seconds > 0 || item.distanceKm || item.averageHeartRate)).sort((a, b) => a.fullDate.localeCompare(b.fullDate)).slice(-12).map((item, index) => ({ ...item, pointLabel: `${item.date} · #${index + 1}` }));
}

function formatWearableSummary(metrics) {
    if (!metrics) return '';
    const duration = metrics.durationSeconds ? formatTime(metrics.durationSeconds) : null;
    const distance = metrics.distanceKm ? `${metrics.distanceKm.toFixed(2)} km` : null;
    return [duration, distance, metrics.pace, metrics.averageHeartRate ? `FC ${Math.round(metrics.averageHeartRate)} bpm` : null].filter(Boolean).join(' · ');
}
...
function forestalCompleteSeries(sessions) {
    return sessions.filter((s) => s.type === 'forestal').map((s) => {
        const complete = FORESTAL_EXERCISES.every(({ name }) => storedSeconds(sessionEntry(s, name)?.temps) > 0);
        if (!complete) return null;
        const raw = FORESTAL_EXERCISES.reduce((sum, { name }) => sum + storedSeconds(sessionEntry(s, name)?.temps), 0);
        const seconds = Number(s.officialTimeSeconds) > 0 ? Number(s.officialTimeSeconds) : officialPhysicalTime('forestal', raw, s.penalties);
        return { date: String(s.date || '').slice(5, 10), fullDate: String(s.date || '').slice(0, 10), seconds, label: 'Simulació completa', grade: gradeForTime('forestal', seconds, s.baremCategory || 'resta') };
    }).filter(Boolean).sort((a, b) => a.fullDate.localeCompare(b.fullDate)).slice(-12).map((item, index) => ({ ...item, pointLabel: `${item.date} · #${index + 1}` }));
}

const FORESTAL_EXERCISES = [
    { name: '1. Fase 1 · 8 rectes + 16 llançaments', label: 'Fase 1 · 8 + 16' },
    { name: '2. Fase 2 · 10 rectes + 20 llançaments', label: 'Fase 2 · 10 + 20' },
    { name: '3. Fase 3 · 12 rectes + 24 llançaments', label: 'Fase 3 · 12 + 24' },
];
const STRUCTURAL_EXERCISES = [
    { name: '1. Discos + equilibri + calaix', label: 'Discos + equilibri' },
    { name: '2. Pesos russos + calaix', label: 'Pesos russos' },
    { name: '3. Estirar trineu', label: 'Estirar trineu' },
    { name: '4. Empènyer trineu', label: 'Empènyer trineu' },
    { name: '5. Recorregut en C', label: 'Recorregut en C' },
    { name: '6. Arrossegament de maniquí', label: 'Maniquí 50 kg' },
    { name: '7. Esprint final', label: 'Esprint final' },
];
...
function structuralCompleteSeries(sessions) {
    return sessions.filter(isCompleteStructuralSession).map((s) => {
        const raw = STRUCTURAL_EXERCISES.reduce((sum, { name }) => sum + storedSeconds(sessionEntry(s, name)?.temps), 0);
        const seconds = Number(s.officialTimeSeconds) > 0 ? Number(s.officialTimeSeconds) : officialPhysicalTime('estructural', raw, s.penalties);
        if (!seconds) return null;
...
export default function ProgressPage() {
    ...
    const forestalTram1 = useMemo(() => forestalSeries(sessions, FORESTAL_EXERCISES[0].name, FORESTAL_EXERCISES[0].label), [sessions]);
    const forestalTram2 = useMemo(() => forestalSeries(sessions, FORESTAL_EXERCISES[1].name, FORESTAL_EXERCISES[1].label), [sessions]);
    const forestalTram3 = useMemo(() => forestalSeries(sessions, FORESTAL_EXERCISES[2].name, FORESTAL_EXERCISES[2].label), [sessions]);
    const forestalComplete = useMemo(() => forestalCompleteSeries(sessions), [sessions]);
    const aquaticTimes = useMemo(() => sessions.filter((s) => s.type === 'aquatic' && Number(s.officialTimeSeconds) > 0).map((s) => ({ date: String(s.date || '').slice(5, 10), fullDate: String(s.date || '').slice(0, 10), seconds: Number(s.officialTimeSeconds), grade: gradeForTime('aquatic', Number(s.officialTimeSeconds), s.baremCategory || 'resta') })).sort((a, b) => a.fullDate.localeCompare(b.fullDate)).slice(-12).map((item, index) => ({ ...item, pointLabel: `${item.date} · #${index + 1}` })), [sessions]);
    ...
    const wearableSeries = useMemo(() => wearableActivitySeries(sessions), [sessions]);
