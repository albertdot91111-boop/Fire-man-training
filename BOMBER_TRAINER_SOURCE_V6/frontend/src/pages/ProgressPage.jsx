...
function sessionTotalSeconds(session) {
    // New sessions store the total duration in minutes.
    const duration = numericSeconds(session?.duration);
    ...
    if (complete) {
        const total = numericSeconds(complete.temps);
        if (total > 0) return total < 20 ? total * 60 : total;
    }

    const values = data.flatMap((entry) => {
        const value = entry?.temps;
        if (value === undefined || value === null || value === '') return [];
        return [typeof value === 'number' ? value : legacyMinuteAware(value)];
    });
...
export default function ProgressPage() {
    ...
    const load = async () => {
        ...
        try {
            setSessions(await pb.collection('bt_sessions').getFullList({ sort: '-date', filter: ownerFilter }));
        } catch (err) {
            setSessions([]);
            setLoadError('No puc llegir les sessions. A PocketBase, List i View de bt_sessions han de permetre @request.auth.id != "" && owner = @request.auth.id.');
        }
    ...
    const forestalTimes = useMemo(() => timeSeries(sessions, 'forestal'), [sessions]);
    ...
    const addWeight = async (e) => {
        ...
        try {
            ...
            setW(''); setFat(''); load();
        } catch (err) { setLoadError(err?.message || 'No s\'ha pogut guardar el pes.'); }
    };
    ...
    const addGoal = async (e) => {
        ...
        } catch (err) { setLoadError(err?.message || 'No s\'ha pogut guardar l\'objectiu.'); }
    };
        ...
        return (
            <section className="rounded-3xl bg-white border border-slate-200 p-5 shadow-sm">
                <div className="flex items-start justify-between gap-3">
                    <div><h2 className="text-lg font-extrabold">{title}</h2><p className="mt-1 text-sm text-slate-500">Evolució del temps total. Com menys, millor.</p></div>
                    {latest && <div className="text-right"><p className="text-xs font-bold text-slate-400">ÚLTIM</p><p className="text-xl font-extrabold">{formatTime(latest.seconds)}</p><p className="text-xs font-bold text-slate-600">{latestGrade ? `Nota aprox. ${latestGrade}` : 'Nota pendent de barem'}</p></div>}
                </div>
                {series.length ? (
                    <>
                        <div className="mt-4 grid grid-cols-2 gap-2">
                            <div className="rounded-2xl bg-slate-50 p-3"><p className="text-xs font-bold text-slate-400">MILLOR TEMPS</p><p className="text-lg font-extrabold">{formatTime(best)}</p><p className="text-xs text-slate-500">{bestGrade ? `Nota aprox. ${bestGrade}` : 'Sense barem numèric'}</p></div>
    ...
    return (
        <AppShell title="El meu progrés">
            ...
            {renderTimeProgress('Prova aquàtica', 'aquatic', aquaticTimes, 'Registra una prova aquàtica per veure la progressió.')}
