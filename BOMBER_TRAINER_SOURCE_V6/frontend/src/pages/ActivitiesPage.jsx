...
function sessionIdentity(session) {
  ...
  const id = String(w?.activityId ?? '').trim();
  if (id) return `intervals:${id}`;
  const start = String(w?.startDateLocal || session?.date || '').slice(0, 16);
  const name = String(w?.name || '').trim().toLowerCase();
  const type = normalizeType(w?.activityType);
  if (start || name || type) return `fallback:${start}|${name}|${type}`;
  return `row:${session?.id}`;
}
function dedupeSessions(rows) {
  const groups = new Map();
  for (const row of rows) {
    const key = sessionIdentity(row);
  ...
  for (const list of groups.values()) {
    list.sort((a, b) => {
      const aClassified = a?.type && a.type !== 'manteniment' ? 1 : 0;
      const bClassified = b?.type && b.type !== 'manteniment' ? 1 : 0;
      if (aClassified !== bClassified) return bClassified - aClassified;
...
export default function ActivitiesPage() {
  ...
  async function load() {
    ...
    try {
      ...
      setSessions(dedupeSessions(Array.isArray(rows) ? rows : []));
    } catch (error) {
  ...
  return <AppShell title="ACTIVITATS">
    ...
    <section className="rounded-3xl bg-white border border-slate-200 p-5 shadow-sm">
      ...
      <div className="mt-3 flex items-center justify-between gap-3 text-xs font-semibold text-slate-400"><span>{sessions.length} activitats úniques carregades</span><button type="button" onClick={() => load()} className="rounded-lg border border-slate-200 px-3 py-2 text-slate-600">↻ Recarregar</button></div>
    </section>
