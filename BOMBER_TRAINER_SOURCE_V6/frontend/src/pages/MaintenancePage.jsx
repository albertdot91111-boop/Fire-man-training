  ...
  const save = async () => {
    ...
    try {
      const data = EXERCISES
        ...
        .map((exercise) => ({ exercici: exercise.name, categoria: exercise.group, llastKg: String(exercise.weight || '').trim(), repeticions: String(exercise.value || '').trim(), temps: String(exercise.time || '').trim() }));
  ...
  const renderSection = (group) => {
    ...
    return <section key={group.title} className="overflow-hidden rounded-3xl border border-slate-200 bg-slate-50 shadow-sm">
      <button type="button" onClick={() => toggleGroup(group.title)} aria-expanded={isOpen} className="flex w-full items-center justify-between gap-3 p-5 text-left">
        <div><h2 className="text-lg font-extrabold text-slate-900">{group.title}</h2>{group.subtitle && <p className="text-sm text-slate-500">{group.subtitle}</p>}</div>
  ...
  return <AppShell title="Manteniment">
    ...
    <div className="mt-4 space-y-5">
      ...
      <section className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
        <button type="button" onClick={() => setGeneralOpen((v) => !v)} aria-expanded={generalOpen} className="flex w-full items-center justify-between gap-3 p-5 text-left">
          <div><h2 className="text-lg font-extrabold text-slate-900">💪 MANTENIMENT GENERAL</h2><p className="text-sm text-slate-500">Força general per mantenir-te preparat.</p></div>
          <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-slate-100 text-xl font-bold text-slate-700 shadow-sm">{generalOpen ? '−' : '+'}</span>
        </button>
        {generalOpen && <div className="space-y-3 border-t border-slate-200 bg-slate-50 p-4">{GENERAL_GROUPS.map(renderSection)}</div>}
      </section>
    </div>
