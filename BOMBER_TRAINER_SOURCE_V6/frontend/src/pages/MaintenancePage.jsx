...
export default function MaintenancePage() {
  ...
  const [values, setValues] = useState(initialValues); const [warmupValues, setWarmupValues] = useState(() => Object.fromEntries(WARMUP.map((_, i) => [i, { reps: '', time: '' }])));
  ...
  const save = async () => {
    ...
    try {
      const warmupData = WARMUP.map(([name, target, rest], i) => ({ exercici: name, categoria: 'ESCALFAMENT', mode: 'training', objectiu: target, descans: rest, repeticions: warmupValues[i].reps, temps: warmupValues[i].time })).filter((x) => x.repeticions || x.temps);
      const exerciseData = EXERCISES.flatMap((exercise) => values[exercise.name].map((series, index) => ({ exercise, series, index })).filter(({ series }) => series.value || series.time || series.weight || series.bodyweight).map(({ exercise, series, index }) => ({ exercici: exercise.name, categoria: exercise.group, serie: index + 1, mode: 'training', llastKg: series.bodyweight ? 'pes corporal' : String(series.weight || '').trim(), repeticions: String(series.value || '').trim(), temps: String(series.time || '').trim() })));
      const data = [...warmupData, ...exerciseData];
      if (!data.length) { setError('Registra almenys un exercici abans de guardar.'); return; }
      await pb.collection('bt_sessions').create({ type: 'manteniment', date: sessionDate, duration: Number(duration) || 0, points: 1, incidents: '', notes, data, owner: pb.authStore.record.id }); navigate('/progres');
