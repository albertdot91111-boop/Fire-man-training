...
export default function TrainPage() {
    ...
    const save = async (kind) => { setBusy(true); setError(''); try { const points = kind === 'complet' ? POINTS.complet : kind === 'manteniment' ? POINTS.manteniment : POINTS.minim; const data = activePlan.map((p, i) => { if (!isMaintenance) { if (isForestal && p.name === 'CIRCUIT COMPLET') return { exercici: p.name, temps: forestalTotalSeconds, tram1: parseTime(entries[0]?.temps), tram2: parseTime(entries[1]?.temps), tram3: parseTime(entries[2]?.temps) }; return Object.fromEntries(Object.entries({ exercici: p.name, ...entries[i] }).map(([key, value]) => [key, TIME_FIELDS.has(key) ? parseTime(value) : value])); } return { exercici: exerciseNames[i].trim() || p.name, llastKg: String(maintenanceWeights[i] ?? '').trim(), series: (entries[i]?.series || []).slice(0, MAINTENANCE_SERIES).map((v) => String(v ?? '').trim()) }; }); if (isMaintenance && !data.some((e) => e.series.some((v) => v !== ''))) { setError('Registra almenys una sèrie abans de guardar.'); return; } await pb.collection('bt_sessions').create({ type: t.key, date: sessionDate, duration: isForestal ? forestalTotalSeconds / 60 : Number(duration) || 0, points: t.key === 'descans' ? 0 : points, incidents: incidents.join(', '), notes, data, owner: pb.authStore.record.id }); navigate('/progres'); } catch (err) { setError(err?.message || 'No s\'ha pogut guardar la sessió.'); } finally { setBusy(false); } };
