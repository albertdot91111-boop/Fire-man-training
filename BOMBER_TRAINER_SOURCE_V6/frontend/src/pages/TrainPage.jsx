...
const FIELD_LABELS = {
    pes: 'Pes (kg)', reps: 'Repeticions', series: 'Sèries', temps: 'Temps (min)', descans: 'Descans (s)',
    tram1: 'Tram 1 (min)', tram2: 'Tram 2 (min)', tram3: 'Tram 3 (min)',
};
...
export default function TrainPage() {
    ...
    const save = async (kind) => {
        ...
        try {
            ...
            navigate('/progres');
        } catch (err) { setError(err?.message || 'No s\'ha pogut guardar la sessió.'); }
        ...
        <AppShell title={t.label}>
            ...
            {isMaintenance && <section className="rounded-3xl bg-white border border-slate-200 p-5 shadow-sm">
                ...
                <div className="mt-3 grid grid-cols-4 gap-2">
                    {MAINTENANCE_MINUTES.map((minutes) => <button key={minutes} type="button" data-testid={`maintenance-duration-${minutes}`} onClick={() => selectMaintenanceMinutes(minutes)} className={`min-h-[56px] rounded-2xl px-2 text-sm font-extrabold transition active:scale-[0.98] ${Number(duration) === minutes ? 'bg-yellow-400 text-slate-900 shadow-sm' : 'bg-slate-100 text-slate-700'}`}>⚡ {minutes} MIN</button>)}
                </div>
                <p className="mt-3 text-sm text-slate-500">Registra tu mateix què has fet. {Number(duration) === 20 ? 'Fins a 4 sèries per exercici.' : Number(duration) === 10 || Number(duration) === 15 ? 'Fins a 3 sèries per exercici.' : '1 sèrie per exercici.'}</p>
            </section>}
            ...
            <section className="space-y-3">
                {activePlan.map((p, i) => <div key={`${p.name}-${i}`} className="rounded-3xl bg-white border border-slate-200 p-4 shadow-sm">
                    {isMaintenance ? <>
                        ...
                        <p className="font-extrabold">{p.name}</p><p className="text-sm text-slate-500">Els 3 trams seguits. El temps total es calcula automàticament.</p>
                        <div className="mt-3 rounded-2xl bg-orange-50 border border-orange-100 p-4"><p className="text-xs font-bold text-orange-700">TEMPS TOTAL</p><p className="mt-1 text-2xl font-extrabold text-slate-900">{formatTime(forestalTotalSeconds)}</p><p className="mt-1 text-xs text-slate-500">Tram 1 + Tram 2 + Tram 3</p></div>
                    </> : <>
                        <p className="font-extrabold">{p.name}</p><p className="text-sm text-slate-500">{p.detail}</p>
                        <div className="mt-3 grid grid-cols-2 gap-3">{p.fields.map((f) => <label key={f} className="grid gap-1 text-sm font-semibold capitalize">{FIELD_LABELS[f] || f}<input type={TIME_FIELDS.has(f) ? 'text' : 'number'} inputMode="decimal" placeholder={TIME_FIELDS.has(f) ? 'ex. 3.5 o 3:30' : undefined} data-testid={`train-field-${i}-${f}`} value={entries[i]?.[f] ?? ''} onChange={(e) => setField(i, f, e.target.value)} className="min-h-[48px] rounded-xl border border-slate-300 px-3" /></label>)}</div>
                    </>}
                </div>)}
            </section>

            <section className="rounded-3xl bg-white border border-slate-200 p-5 shadow-sm space-y-4">
                {isMaintenance ? <p className="text-sm font-semibold">Durada seleccionada: <strong>{duration} min</strong></p> : isForestal ? <p className="text-sm font-semibold">Durada total: <strong>{formatTime(forestalTotalSeconds)}</strong></p> : <label className="grid gap-1 text-sm font-semibold">Durada total (min)<input type="number" step="0.1" value={duration} onChange={(e) => setDuration(e.target.value)} className="min-h-[48px] rounded-xl border border-slate-300 px-3" /></label>}
                ...
                <div className="grid gap-2 sm:grid-cols-3">{!isMaintenance && <button type="button" data-testid="train-save-complet" disabled={busy} onClick={() => save('complet')} className="min-h-[52px] rounded-xl bg-slate-900 font-bold text-white active:scale-[0.98]">Complet +100</button>}<button type="button" data-testid="train-save-manteniment" disabled={busy} onClick={() => save('manteniment')} className="min-h-[52px] rounded-xl bg-yellow-400 font-bold text-slate-900 active:scale-[0.98]">{isMaintenance ? 'Guardar manteniment +40' : 'Manteniment +40'}</button><button type="button" data-testid="train-save-minim" disabled={busy} onClick={() => save('minim')} className="min-h-[52px] rounded-xl bg-slate-100 font-bold text-slate-700 active:scale-[0.98]">{isMaintenance ? 'Guardar mínim +20' : 'Mínim +20'}</button></div>
