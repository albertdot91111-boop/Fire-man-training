...
export default function TrainPage() {
        ...
        try {
            ...
            const data = activePlan.map((p, i) => {
                if (!isMaintenance) {
                    ...
                    if (mode === 'training') {
                        base.temps = '';
                        base.repeticions = String(entries[i]?.trainingReps ?? '').trim();
                        delete base.trainingReps;
                    }
                    return Object.fromEntries(Object.entries(base).map(([key, value]) => [key, isPressBench && key === 'temps' ? (Number(value) || 0) : (TIME_FIELDS.has(key) ? parseTrainingTime(value) : value)]));
                }
    ...
    return <AppShell title={t.label}>
        ...
        <section className="space-y-3">
            {activePlan.map((p, i) => {
                const mode = entries[i]?.mode || 'official';
                const switchMode = canSwitchMode(p);
                const fields = mode === 'training' ? p.fields.filter((f) => f !== 'temps') : p.fields;
                return <div key={`${p.name}-${i}`} className="rounded-3xl bg-white border border-slate-200 p-4 shadow-sm">
                    {isMaintenance ? <>
                        ...
                        <label className="mt-3 grid gap-1 text-sm font-semibold">Pes llast (kg) <span className="text-xs font-normal text-slate-400">Opcional</span><input type="number" min="0" step="0.5" placeholder="Sense llast" value={maintenanceWeights[i] ?? ''} onChange={(e) => setMaintenanceWeight(i, e.target.value)} className="min-h-[48px] rounded-xl border border-slate-300 px-3" /></label>
                        <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4">{Array.from({ length: MAINTENANCE_SERIES }, (_, s) => <label key={s} className="grid gap-1 text-xs font-bold text-slate-500">Sèrie {s + 1}<input type="text" inputMode="numeric" value={entries[i]?.series?.[s] ?? ''} onChange={(e) => setMaintenanceSeries(i, s, e.target.value)} className="min-h-[52px] w-full rounded-xl border border-slate-300 px-3 text-base font-extrabold" /></label>)}</div>
                        ...
                        <div className="mt-3 rounded-2xl bg-orange-50 p-4"><p className="text-xs font-bold">TEMPS TOTAL</p><p className="mt-1 text-2xl font-extrabold">{formatTime(forestalTotalSeconds)}</p><p className="mt-1 text-sm font-extrabold text-orange-700">{forestalCompletedTrams}/3 trams completats{forestalComplete ? ' · prova completa' : ' · prova parcial'}</p>{forestalGrade !== null && <p className="mt-1 text-sm font-extrabold text-orange-700">GLOBAL · {forestalGrade}/10 · {Math.round(forestalGrade * 10)}%</p>}</div>
                    </> : <>
                        <div className="flex items-center justify-between gap-3">
                            <div className="min-w-0"><p className="font-extrabold">{p.name}</p><p className="text-sm text-slate-500">{p.detail}</p></div>
                            {switchMode && <select aria-label={`Mode ${p.name}`} value={mode} onChange={(e) => setExerciseMode(i, e.target.value)} className="min-h-[42px] shrink-0 rounded-xl border border-slate-300 bg-white px-3 text-xs font-extrabold text-slate-700"><option value="official">Prova oficial</option><option value="training">Entrenament</option></select>}
                        </div>
                        {isStructural && <StructuralExerciseGraphic kind={structuralGraphicKind(p.name)} />}
                        <div className="mt-3 grid grid-cols-2 gap-3">
                            {mode === 'training' && <label className="grid gap-1 text-sm font-semibold">Sèries / cops<input type="number" min="0" step="1" inputMode="numeric" value={entries[i]?.trainingReps ?? ''} onChange={(e) => setField(i, 'trainingReps', e.target.value)} className="min-h-[48px] rounded-xl border border-slate-300 px-3" /></label>}
