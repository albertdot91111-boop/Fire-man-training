...
const AuthenticatedApp=()=>{const {user,isLoadingAuth,isLoadingPublicSettings,authError,navigateToLogin}=useAuth();if(isLoadingPublicSettings||isLoadingAuth)return <RouteFallback/>;if(authError){if(authError.type==='user_not_registered')return <UserNotRegisteredError/>;if(authError.type==='auth_required'){navigateToLogin();return null;}}return <Suspense fallback={<RouteFallback/>}><Routes><Route path="/login" element={<LoginPage/>}/><Route path="/register" element={<Register/>}/><Route path="/forgot-password" element={<ForgotPassword/>}/><Route path="/reset-password" element={<ResetPassword/>}/><Route path="/oauth/callback" element={<OAuthCallback/>}/><Route path="/legal/:type" element={<LegalPage/>}/><Route path="/admin/accessos" element={<AdminAccessPage/>}/><Route path="/admin/progres-usuaris" element={<AdminUsersProgressPage/>}/><Route element={<ProtectedRoute unauthenticatedElement={<Navigate to="/login" replace/>}/> }><Route path="/" element={<HomePage/>}/><Route path="/entrena/manteniment" element={<MaintenancePage/>}/><Route path="/pautes-entrenador" element={<TrainerPlansPage/>}/><Route path="/nutricio" element={<NutritionPage/>}/><Route path="/base-forca" element={<BaseStrengthPage/>}/><Route path="/entrena/rapid" element={<GuidedRapidWorkout/>}/><Route path="/entrena/:type" element={<TrainPage/>}/><Route path="/progres" element={<ProgressPage/>}/><Route path="/perfil" element={<ProfilePage/>}/><Route path="/ia" element={<AiPage/>}/><Route path="/activitats" element={<ActivitiesPage/>}/><Route path="/configuracio" element={<SettingsPage/>}/></Route><Route path="*" element={<PageNotFound/>}/></Routes></Suspense>};
function App(){const [legalAccepted,setLegalAccepted]=React.useState(hasLegalConsent);return <AuthProvider><QueryClientProvider client={queryClientInstance}><Router><ScrollToTop/><AuthenticatedApp/><AdminProgressEye/><NutritionDaily/><ProgressVisualFixes/>{!legalAccepted && <LegalConsentGate onAccepted={() => setLegalAccepted(true)} />}</Router><Toaster/></QueryClientProvider></AuthProvider>}
export default App;
