...
const FREE_MEAL_MESSAGES = [
  '😎 Avui toca gaudir! És el teu àpat lliure de la setmana. Sense culpa. Demà tornem al pla. 🔥',
  '🍕 Àpat lliure activat! Gaudeix-lo. Un àpat no espatlla una bona setmana. 💪',
  '❤️ La constància no és fer-ho perfecte. Gaudeix del teu àpat lliure i demà continuem.',
  '😏 Avui hi ha capritx. Perfecte! Gaudeix-lo i demà tornem a la rutina.',
];
...
function readStatus(owner, date) {
  try { const value = localStorage.getItem(storageKey(owner, date)); return ['good', 'bad', 'free_meal'].includes(value) ? value : null; } catch (_) { return null; }
}
...
export default function NutritionDaily() {
  ...
  useEffect(() => { setStatus(readStatus(owner, today)); }, [owner, today]);
  useEffect(() => {
    if (!due) return;
    setOpen(true);
    if (permission === 'granted') {
      ...
      if (!already) {
        try { new Notification('🍽️ Com has anat avui?', { body: 'Marca el teu dia o gaudeix del teu àpat lliure de la setmana.', tag: 'bt-nutrition-daily' }); } catch (_) {}
  ...
  const save = (value) => {
    if (value === 'free_meal') useFreeMeal(owner, week);
  ...
  const requestNotifications = async () => {
    ...
    try { setPermission(await Notification.requestPermission()); } catch (_) {}
  };
  ...
  useEffect(() => {
    const decorateCalendar = () => {
      ...
      Array.from(grid.querySelectorAll('button')).forEach((button) => {
        button.querySelector('[data-bt-nutrition-marker]')?.remove();
        const day = Number(button.querySelector('span')?.textContent?.trim());
        if (!Number.isInteger(day) || day < 1 || day > 31) return;
        const base = new Date(now.getFullYear(), now.getMonth(), 1);
        const key = dateKey(new Date(base.getFullYear(), base.getMonth(), day));
        const markerStatus = readStatus(owner, key);
        ...
        marker.style.cssText = `position:absolute;right:4px;bottom:3px;width:18px;height:18px;border-radius:9999px;display:flex;align-items:center;justify-content:center;font-size:${markerStatus === 'free_meal' ? '11px' : '11px'};font-weight:900;color:white;background:${markerStatus === 'good' || markerStatus === 'free_meal' ? '#16a34a' : '#dc2626'};box-shadow:0 1px 3px rgba(0,0,0,.18);z-index:5;`;
  ...
  return <>
    ...
    {open && due && <div className="fixed inset-0 z-[100] flex items-end justify-center bg-slate-950/40 p-4 sm:items-center" role="dialog" aria-modal="true" aria-labelledby="nutrition-heading">
      <section className="w-full max-w-md rounded-3xl bg-white p-6 shadow-2xl">
        <div className="flex items-start justify-between gap-3"><div><p className="text-xs font-bold tracking-[0.18em] text-green-700">CONTROL DIARI</p><h2 id="nutrition-heading" className="mt-1 text-2xl font-extrabold">🍽️ Com has anat avui?</h2></div><span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-bold">20:00</span></div>
        <p className="mt-3 text-sm font-medium text-slate-600">Has seguit el teu pla avui? I recorda: tens <strong>1 àpat lliure per setmana</strong>.</p>
        <p className="mt-3 rounded-2xl bg-slate-50 p-3 text-sm font-bold text-slate-700">{status === 'free_meal' ? FREE_MEAL_MESSAGES[now.getDate() % FREE_MEAL_MESSAGES.length] : MOTIVATION[now.getDate() % MOTIVATION.length]}</p>
        <div className={`mt-5 grid ${freeMealUsed ? 'grid-cols-2' : 'grid-cols-3'} gap-2`}>
          <button type="button" onClick={() => save('good')} className="min-h-[64px] rounded-2xl bg-green-600 px-2 text-sm font-extrabold text-white">🟢 Pla complert</button>
          {!freeMealUsed && <button type="button" onClick={() => save('free_meal')} className="min-h-[64px] rounded-2xl bg-amber-500 px-2 text-sm font-extrabold text-white">🍕 Àpat lliure</button>}
          <button type="button" onClick={() => save('bad')} className="min-h-[64px] rounded-2xl bg-red-600 px-2 text-sm font-extrabold text-white">🔴 Fora del pla</button>
        </div>
        {freeMealUsed && <p className="mt-3 text-center text-xs font-bold text-amber-700">🍕 Ja has utilitzat l'àpat lliure d'aquesta setmana.</p>}
        ...
        <p className="mt-3 text-center text-xs text-slate-400">🟢 dia complert · 🍕 àpat lliure · 🔴 dia fora del pla</p>
      </section>
    </div>}
  </>;
}
