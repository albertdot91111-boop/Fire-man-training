...
async function request(action, params = {}) {
  ...
  for (let attempt = 0; attempt < 4; attempt += 1) {
    ...
    // A 429 is a provider-level hourly limit. Retrying/splitting only creates
    // more requests and makes the situation worse, so surface it immediately.
...
export async function deleteAllIntervalsActivities({pb,owner,onProgress}){
  ...
  for(let i=0;i<imported.length;i+=1){
    ...
    for(let attempt=0;attempt<3;attempt+=1){
      try{await pb.collection('bt_sessions').delete(imported[i].id);deleted+=1;done=true;break;}catch(error){lastError=error;const status=Number(error?.status||error?.response?.code||0);if(status===401||status===403)throw error;if(status===429)break;if(attempt<2)await sleep(350*(attempt+1));}}
