...
function dateChunks(days) {
  const end = new Date(); const start = new Date(end.getTime() - days * 86400000); const chunks = [];
  for (let cursor = start; cursor <= end;) {
    const chunkEnd = new Date(Math.min(cursor.getTime() + 364 * 86400000, end.getTime()));
...
async function fetchChunkResilient(chunk, onProgress, label = '') {
  try { return { rows: await request('activities', chunk), failedRanges: [] }; }
  catch (error) {
    const from = new Date(`${chunk.oldest}T00:00:00Z`).getTime(); const to = new Date(`${chunk.newest}T00:00:00Z`).getTime();
    ...
    const left = { oldest: chunk.oldest, newest: middle.toISOString().slice(0, 10) }; const rightStart = new Date(middle.getTime() + 86400000); const right = { oldest: rightStart.toISOString().slice(0, 10), newest: chunk.newest };
    onProgress?.(`${label}: reintentant rang petit`);
    const [a, b] = await Promise.all([fetchChunkResilient(left, onProgress, label), fetchChunkResilient(right, onProgress, label)]);
    return { rows: [...a.rows, ...b.rows], failedRanges: [...a.failedRanges, ...b.failedRanges] };
  }
}
export async function getRecentIntervalsActivities(days = 3650, onProgress) {
  const all = []; const seen = new Set(); const failedRanges = []; const chunks = dateChunks(days);
  for (let i = 0; i < chunks.length; i += 1) {
    ...
    for (const activity of Array.isArray(result.rows) ? result.rows : []) {
      const id = String(activity?.id ?? '').trim(); const fallback = `${activity?.start_date_local || ''}|${activity?.name || ''}|${activity?.type || ''}`; const key = id ? `id:${id}` : `fb:${fallback}`;
      if (!seen.has(key)) { seen.add(key); all.push(activity); }
    }
    failedRanges.push(...result.failedRanges); if (i < chunks.length - 1) await sleep(500);
  }
...
export async function syncRecentIntervalsActivities({ pb, owner, days = 3650, typeResolver, onProgress }) {
  ...
  for (const row of existingRows) {
    const wearable = parseWearable(row?.wearable); const record = { row, wearable, sourced: wearable?.source === 'intervals.icu' };
    const id = wearableIdKey(wearable); if (id) byId.set(id, record); // IMPORTANT: index every row, not only Intervals rows.
    const date = String(row?.date || wearable?.startDateLocal || '').slice(0, 10); if (date) { const list = rowsByDate.get(date) || []; list.push(record); rowsByDate.set(date, list); }
  }
  ...
  for (const activity of activities) {
    try {
      ...
      let existingRecord = id ? byId.get(id) : null;
      if (existingRecord && consumed.has(existingRecord.row.id)) existingRecord = null;
      const candidates = (rowsByDate.get(date) || []).filter((r) => !consumed.has(r.row.id));
      // Exact timestamp + name + type is the strongest non-ID match.
      ...
      // Then timestamp + type, useful for Suunto/manual rows whose name differs.
      ...
      // Then exact date/name/type, but only if unique.
      ...
      if (existingRecord) {
        ...
        const patch = { date, wearable }; if (wearable.durationSeconds) patch.duration = Math.round((wearable.durationSeconds / 60) * 10) / 10; if (suggestedType && existingRecord.row.type === 'manteniment') patch.type = suggestedType;
        await pb.collection('bt_sessions').update(existingRecord.row.id, patch); updated += 1; continue;
      }
      const wearable = buildWearable(activity); const created = await pb.collection('bt_sessions').create({ type: suggestedType || 'manteniment', date, duration: wearable.durationSeconds ? Math.round((wearable.durationSeconds / 60) * 10) / 10 : 0, points: 0, notes: 'Activitat sincronitzada des d’Intervals.icu · pendent d’associar', data: [], wearable, owner });
      const record = { row: created, wearable, sourced: true }; consumed.add(created.id); if (id) byId.set(id, record); const list = rowsByDate.get(date) || []; list.push(record); rowsByDate.set(date, list); imported += 1;
